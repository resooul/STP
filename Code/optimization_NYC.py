...
data_X_Train = copy.deepcopy(X_Train)
data_X_Test = copy.deepcopy(X_Test)
...
for i, rowX in X_Train.iterrows():
    ...
    shipmentCreatedDate = dt.datetime.strptime(datasetRow.at[index, 'start_datetime'], "%Y-%m-%d %H:%M:%S")
    shipmentStatusDate = dt.datetime.strptime(datasetRow.at[index, 'statusDate'], "%Y-%m-%d %H:%M:%S")
    ...
    optimizer = STPMPA.STPMPA(epoch=25, pop_size=5)
...
for i, rowX in X_Test.iterrows():
    ...
    optimizer = MPA.STPMPA(epoch=25, pop_size=5)
